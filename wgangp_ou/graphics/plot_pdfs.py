...
if read_gen:
    ...
    v_bin = stats.create_log_bins(-0.45,0.45,600,1.e-3) #VAR
    a_bin = stats.create_log_bins(-0.011,0.011,600,5.e-6) #VAR
    r_bin = stats.create_log_bins(-0.0015,0.0015,600,1.e-6) #VAR
    v_for_norm = []
    a_for_norm = []
    r_for_norm = []
...
if not comp_real and not only_hist:
    ...
    bins = stats.create_log_bins(-0.45,0.45,600,1.e-3) #VAR
...
if not comp_real and not only_hist:
    ...
    # BINS LOGARITMICI FISSI
    bins = stats.create_log_bins(-0.011,0.011,600,5.e-6) #VAR
...
if not comp_real and not only_hist:
    ...
    # bins = np.linspace(-6,6,2000)
    # BINS LOGARITMICI FISSI
    bins = stats.create_log_bins(-0.0015,0.0015,600,1.e-6) #VAR
